...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      switch (transcriptionService) {
        case 'deepgram':
          ...
          transcriptionResult = await retryTranscriptionCall(() => 
            callDeepgram(audioSource, transcriptionModel, process.env.DEEPGRAM_API_KEY!)
          )
          break
        ...
        case 'assembly':
          ...
          transcriptionResult = await retryTranscriptionCall(() => 
            callAssembly(audioSource, transcriptionModel, process.env.ASSEMBLY_API_KEY!)
          )
          break
        ...
        case 'groq':
          ...
          transcriptionResult = await retryTranscriptionCall(() => 
            callGroq(audioSource, transcriptionModel, process.env.GROQ_API_KEY!)
          )
          break
      ...
      console.error(`[API] Transcription failed for ${transcriptionService}:`, error)
      throw error
    }
