...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (type === 'video' && !url) {
      return NextResponse.json(
        { error: 'URL is required for video' },
        { status: 400 }
      )
    }
    ...
    if (type === 'video') {
      ...
      const { stdout } = await execFilePromise(ytDlpPath, [
        ...
        'chrome', // 尝试从 Chrome 浏览器获取 cookies
        '--print',
        '%(webpage_url)s',
        '--print',
        '%(channel)s',
        ...
        try {
          ...
          if (
            ...
            if (!args.includes('--cookies-from-browser')) {
              const insertIndex =
                args.findIndex(arg => arg.startsWith('--')) + 2
              newArgs.splice(
                insertIndex,
                0,
                '--cookies-from-browser',
    ...
    if (type === 'video') {
      ...
      if (!fileType || !supportedFormats.has(fileType.ext)) {
        const errorMsg = fileType
          ? `Unsupported file type: ${fileType.ext}`
          : 'Unable to determine file type'
        throw new Error(errorMsg)
      }

      await execPromise(
        `ffmpeg -i "${tempFilePath}" -ar 16000 -ac 1 -c:a pcm_s16le "${outputPath}"`,
    ...
    l(
      `${pre} Computing transcription costs using duration: ${audioDurationInSeconds} seconds`
    )
    const transcriptionCosts = await computeTranscriptionCosts(outputPath)

    // Upload audio file using storage service
    ...
    l(`${pre} File uploaded to storage`)
    ...
    // Get audio file URL
    const s3Url = await storageService.getAudioSignedUrl(key)
    l(`${pre} Generated audio file URL`)

    // Create show note
    l(`${pre} Creating show note`)
    const { id } = await storageService.createShowNote({
      title: metadata.title,
      publishDate: metadata.publishDate,
      ...
      return NextResponse.json(
        {
          ...
          details:
            '由于 YouTube 的反机器人保护，无法下载此视频。建议：1) 尝试使用其他 YouTube 视频 URL，2) 或者下载视频文件后直接上传处理',
