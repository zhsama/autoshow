...
import { err, l, L_CONFIG } from '@autoshow/shared'
...
export async function callChatGPT(
  ...
  l(
    `${pre} Preparing request with combined prompt length: ${combinedPrompt.length}`
  )
  ...
  try {
    ...
    l(
      `${pre} Successfully processed response, content length: ${content.length}`
    )
...
export async function callGemini(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      l(
        `${pre} Sending request to Gemini API (attempt ${attempt}/${maxRetries})`
      )
      ...
      l(
        `${pre} Successfully processed response, content length: ${text.length}`
      )
      ...
      err(
        `${pre} Error in callGemini (attempt ${attempt}/${maxRetries}):`,
        error
      )
...
export async function computeLLMCosts(
  transcriptLength: number,
  promptLength: number
): Promise<
  Record<string, Array<{ modelId: string; modelName: string; cost: number }>>
> {
  const pre = '[llm.service:computeLLMCosts]'
  l(
    `${pre} Computing LLM costs for transcript length: ${transcriptLength}, prompt length: ${promptLength}`
  )

  const totalInputTokens = Math.max(
    1,
    Math.ceil((transcriptLength + promptLength) / 4)
  )
  const estimatedOutputTokens = 4000

  l(
    `${pre} Estimated input tokens: ${totalInputTokens}, estimated output tokens: ${estimatedOutputTokens}`
  )

  const result: Record<
    string,
    Array<{ modelId: string; modelName: string; cost: number }>
  > = {}
  ...
  Object.entries(L_CONFIG).forEach(([serviceName, config]) => {
    ...
    config.models.forEach(model => {
      ...
      const totalCost = Number.parseFloat((inputCost + outputCost).toFixed(10))

      l(
        `${pre} Model: ${model.modelId}, Input cost: ${inputCost}, Output cost: ${outputCost}, Total cost: ${totalCost}`
      )
