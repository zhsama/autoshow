...
import { s3Service } from '@autoshow/shared/server'
  ...
  try {
    ...
    try {
      ...
      if (llmResult.usage && llmService !== 'ollama') {
        const modelConfig = L_CONFIG[llmService as keyof typeof L_CONFIG]?.models.find(
          m => m.modelId === llmModel
        )
        
        if (modelConfig && 'inputCostC' in modelConfig && 'outputCostC' in modelConfig) {
          const inputCost = (llmResult.usage.input || 0) * modelConfig.inputCostC / 100000
          const outputCost = (llmResult.usage.output || 0) * modelConfig.outputCostC / 100000
