...
export function formatGroqTranscript(result: GroqTranscriptionResponse): string {
  ...
  if (result.segments && result.segments.length > 0) {
    l(`${pre}:formatGroqTranscript Found ${result.segments.length} segments to format`)
    
    let txtContent = ''
    
    result.segments.forEach((segment, index) => {
      l(`${pre}:formatGroqTranscript Processing segment ${index+1}/${result.segments!.length}`)
      const timestamp = formatTimestamp(segment.start)
      txtContent += `[${timestamp}] ${segment.text.trim()}\n`
    })
    
    txtContent += '\n'
    l(`${pre}:formatGroqTranscript Formatting complete with timestamps, content length: ${txtContent.length}`)
    return txtContent
  }
...
export async function callGroq(
  ...
  const modelInfo = T_CONFIG.groq.models.find(m => m.modelId.toLowerCase() === groqModel.toLowerCase())
  if (!modelInfo) {
    err(`${methodLogPrefix} Model information for model ${groqModel} is not defined`)
    throw new Error(`Model information for model ${groqModel} is not defined.`)
  }
  
  const { modelId, costPerMinuteCents } = modelInfo
  l(`${methodLogPrefix} Using Groq model: ${modelId} with cost: ¢${costPerMinuteCents} per minute`)
  ...
  if (audioBuffer.length > maxSizeBytes) {
    err(`${methodLogPrefix} Audio file size ${audioBuffer.length} exceeds Groq limit of ${maxSizeBytes} bytes`)
    throw new Error(`Audio file size exceeds Groq's 25MB limit. Please use a smaller file or split it.`)
  }
  
  l(`${methodLogPrefix} Sending ${audioBuffer.length} bytes to Groq API with model ${modelId}`)
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Failed to read Groq response: ${e instanceof Error ? e.message : String(e)}`)
    }
    ...
    try {
      ...
      throw new Error(`Invalid JSON in Groq response: ${e instanceof Error ? e.message : String(e)}. Response text: ${responseText.substring(0, 100)}...`)
    }
